...
describe('filterToolsForGrant', () => {
  ...
  it('hides project-agnostic tools in project-scoped mode', () => {
    ...
    const names = tools.map((t) => t.name);
    expect(tools).toHaveLength(36); // 24 upstream + 3 day-one (T6/T8/T2) + feat-057 get_policy + feat-019 explain_plans + feat-020 T4 + feat-021 T5 + feat-025 T12 + feat-045 generate_rca_report (T1 hidden as project-agnostic)
