...
describe('NEON_TOOLS definitions', () => {
  it('has 39 tools (31 upstream + 4 day-one T1/T2/T6/T8 + feat-057 policy + feat-019 explain_plans + feat-020 T4 health_signals + feat-021 T5 query_performance)', () => {
    expect(NEON_TOOLS).toHaveLength(39);
  });
...
describe('tool category field (feat-005 #2 · narrative #3 ecosystem-friendly)', () => {
  ...
  it('current optional count = 35 (39 total - 4 core · keeps 26+ listing budget for ecosystem MCPs)', () => {
    const optional = NEON_TOOLS.filter((t) => t.category === 'optional');
    expect(optional.length).toBe(35);
  });
