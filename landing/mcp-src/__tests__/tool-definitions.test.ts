...
describe('NEON_TOOLS definitions', () => {
  it('has 40 tools (31 upstream + 4 day-one T1/T2/T6/T8 + feat-057 policy + feat-019 explain_plans + feat-020 T4 health_signals + feat-021 T5 query_performance + feat-024 T11 query_samples)', () => {
...
describe('tool category field (feat-005 #2 · narrative #3 ecosystem-friendly)', () => {
  ...
  it('current optional count = 36 (40 total - 4 core · keeps 26+ listing budget for ecosystem MCPs)', () => {
    const optional = NEON_TOOLS.filter((t) => t.category === 'optional');
    expect(optional.length).toBe(36);
  });
