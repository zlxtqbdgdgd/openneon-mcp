...
describe('NEON_TOOLS definitions', () => {
  it('has 44 tools (31 upstream + 4 day-one T1/T2/T6/T8 + feat-057 policy + feat-019 T3 + feat-020 T4 + feat-021 T5 + feat-022 T7 + feat-023 T10 + feat-024 T11 + feat-025 T12 + feat-045 generate_rca_report)', () => {
    expect(NEON_TOOLS).toHaveLength(44);
  });
...
describe('tool category field (feat-005 #2 · narrative #3 ecosystem-friendly)', () => {
  ...
  it('current optional count = 40 (44 total - 4 core · feat-045 RCA +1 optional · keeps 26+ listing budget for ecosystem MCPs)', () => {
    const optional = NEON_TOOLS.filter((t) => t.category === 'optional');
    expect(optional.length).toBe(40);
  });
