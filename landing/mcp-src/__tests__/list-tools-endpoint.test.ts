...
describe('/api/list-tools endpoint', () => {
  ...
  it('returns 41 tools when include=all (full listing opt-in · backward-compat for clients wanting upstream tools)', async () => {
    const body = await callListTools({ include: 'all' });
    expect(body.categoryInclude).toBe('all');
    expect(body.tools).toHaveLength(41); // 31 upstream + 4 day-one (T1/T2/T6/T8) + feat-057 get_policy + feat-019 explain_plans + feat-020 T4 + feat-021 T5 + feat-024 T11
  });
  ...
  it('filters by scopes when category param is present (with include=all to isolate grant filter)', async () => {
    ...
    expect(body.tools).toHaveLength(17); // 10 upstream + 2 day-one (T6/T2) + feat-019 explain_plans + feat-020 T4 + feat-021 T5 + feat-024 T11 · scope='querying'
  });
  ...
  it('filters project-agnostic tools in project-scoped mode (with include=all)', async () => {
    ...
    expect(body.tools).toHaveLength(33); // 24 upstream + 3 day-one (T6/T8/T2) + feat-057 get_policy + feat-019 explain_plans + feat-020 T4 + feat-021 T5 + feat-024 T11 (require projectId · T1 hidden)
  ...
  it('filters to readOnlySafe tools with readonly=true (with include=all)', async () => {
    ...
    expect(body.tools).toHaveLength(29); // 19 upstream + 4 day-one (T1/T2/T6/T8) + feat-057 get_policy + feat-019 explain_plans + feat-020 T4 + feat-021 T5 + feat-024 T11 · all readOnlySafe
  ...
  it('supports legacy x-read-only header (with include=all to test header path)', async () => {
    ...
    expect(body.readOnly).toBe(true);
    expect(body.tools).toHaveLength(29); // 19 upstream + 4 day-one (T1/T2/T6/T8) + feat-057 + feat-019 explain_plans + feat-020 T4 + feat-021 T5 + feat-024 T11 · readOnlySafe
  });
