...
describe('/api/list-tools endpoint', () => {
  ...
  it('returns NEON_TOOLS.length tools when include=all (full listing opt-in · backward-compat for clients wanting upstream tools)', async () => {
    ...
    expect(body.tools).toHaveLength(46); // 31 upstream + 4 day-one (T1/T2/T6/T8) + feat-057 get_policy + feat-019 explain_plans + feat-020 T4 + feat-021 T5 + feat-025 T12 pool_stats + feat-045 generate_rca_report + feat-037 cluster_neondb_logs
  });
  ...
  it('filters project-agnostic tools in project-scoped mode (with include=all)', async () => {
    ...
    expect(body.tools).toHaveLength(38); // 24 upstream + 3 day-one (T6/T8/T2) + feat-057 get_policy + feat-019 explain_plans + feat-020 T4 + feat-021 T5 + feat-025 T12 + feat-045 generate_rca_report + feat-037 cluster_neondb_logs (T1 hidden as project-agnostic)
  ...
  it('readonly query param takes precedence over x-read-only header (with include=all)', async () => {
    ...
    const body = (await res.json()) as ListToolsResponse;
    expect(body.readOnly).toBe(false);
    expect(body.tools).toHaveLength(46); // 31 upstream + 4 day-one (T1/T2/T6/T8) + feat-057 get_policy + feat-019 explain_plans + feat-020 T4 + feat-021 T5 + feat-025 T12 pool_stats + feat-045 generate_rca_report
  });
